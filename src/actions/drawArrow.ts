...
class DrawArrowOptions extends ActionOptions {
    ...
    constructor(originX: number, originY: number, destinationX: number, destinationY: number, lineWidth: number, color: string) {
      super();
  ...
  class DrawArrow extends Action<DrawArrowOptions> {
    ...
    do(context: CanvasRenderingContext2D): void {
      ...
      let angle = Math.atan2(lineCanvasDestinationY - lineCanvasOriginY, lineCanvasDestinationX - lineCanvasOriginX);
      context.moveTo(arrowHeadRadius * Math.cos(angle) + lineCanvasDestinationX, arrowHeadRadius * Math.sin(angle) + lineCanvasDestinationY);
      angle += (1.0/3.0) * (2 * Math.PI);
      context.lineTo(arrowHeadRadius * Math.cos(angle) + lineCanvasDestinationX, arrowHeadRadius * Math.sin(angle) + lineCanvasDestinationY);
      angle += (1.0/3.0) * (2 * Math.PI);
      context.lineTo(arrowHeadRadius * Math.cos(angle) + lineCanvasDestinationX, arrowHeadRadius * Math.sin(angle) + lineCanvasDestinationY);
      context.closePath();
